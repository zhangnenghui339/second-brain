---
title: CareerRadar 待办物料库
type: project
project: careerradar
created: 2026-07-17
status: active
---

# 待办物料库

> 存放 [`../_index.md`](../_index.md) 中待办事项的相关物料:参考文档、截图、素材、外部链接的本地副本等。本身不是待办事项,只是待办的支撑材料。

## 物料清单

### ATS 实测(Day 1–14 技术门)· 对应待办见 [`../_index.md`](../_index.md)

| 文件 | 说明 | 状态 |
| ---- | ---- | ---- |
| [母本简历_内容冻结_FROZEN.md](母本简历_内容冻结_FROZEN.md) | Alex Chen 虚构母本简历全文,2026-07-17 冻结,变体制作唯一文本来源 | **FROZEN,禁止改动** |
| Alex-Chen-Resume.docx | V01 基线变体源文件(单栏/Arial 11pt/纯文字) | 母版,V02–V06 由此派生 |
| Alex-Chen-Resume.pdf | V01 导出的 PDF,已按上传统一文件名命名 | 上传用 |
| ATS测试记录表.xlsx | 测试记录表,schema 见 SOP 0.3 节(run_id/ats/variant_id/字段级 pass 等) | 每次上传一行 |

## 命名规则

- 单个物料文件:`任务关键词_内容说明.扩展名`(如 `impact账户限制_teal沟通记录.md`)
- 单个任务物料较多时,建子文件夹:`任务关键词/`,内部文件按需命名
- 物料对应哪条待办,需在 `../_index.md` 表格的"详情/物料"列做链接,保持双向可追溯
