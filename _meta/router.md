...
所有问题先执行 `prompts/_index.md` 的 **Ponytail(Problem)** 前置检查,判断问题是否需要存在、能否被消解或复用已有答案。

若问题包含“本质 / 最核心 / 最重要 / 根本问题 / 世界模型 / 升维 / 窄视野”等语义意图,或用户刚发现一个重要概念并准备把它升级为普遍结论,则在 Ponytail 之后加载 `prompts/persona/世界模型与根本问题审计器.md`:

1. 它是跨域**辅助角色**,负责问题重构、观察尺度、系统边界、模型盲区与反证条件。
2. 它不占用 3.1 节“最多两个领域角色”的名额,也不得替代领域主角色。
3. 执行顺序:`Ponytail → 世界模型审计器(按需) → 领域主角色 → 相关记忆 → 回答`。
4. 简单事实查询、已确定方案的纯执行任务、格式化或改写任务不加载,避免过度分析。
